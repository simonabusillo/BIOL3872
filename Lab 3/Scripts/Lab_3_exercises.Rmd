...
?cbind
new_df <- cbind(iris$Sepal.Length,iris$Species)
...
plot(iris,col=iris$Species) #petal on this plot is the yaxis -> virginica is biggest
#each species is given a factor (species 1=black, species 2=red, species 3=green)
...
#type 'colours' to view all the colour options
```
...
# return objects where species is "setosa" OR (|) "versicolour" -> use & if you want to return something with the both names
...
ggplot(data = iris, aes(x = Petal.Length, y = Sepal.Length, color = Species))+
  geom_point()
# add the data the aesthetics (x-axis=petal length, y-axis = sepal length, colour each point by species), then add type of graph/geometry (point = geometry)

# You can see the legend is automatically added, that is because we specified the color grouping by species in the aesthetic **must include in the aes if you want it to change**
```
...
ggplot(data = iris, aes(x = Petal.Length, y = Sepal.Length, color = Species))+
  ...
  theme_bw(12) + # theme changes the background theme -> do '?theme' to explore themes
...
ggplot(data = iris, aes(x = Petal.Length, y = Sepal.Length))+
  ...
  scale_x_continuous(breaks = seq(0,8,1))+
  guides(color = guide_legend(override.aes = list(alpha = 1))) #overrides transparency in legend and sets them back to normal (1)
```

```{r}
ggplot(data = iris, aes(x = Petal.Length, y = Sepal.Width, fill = Species))+
  geom_violin() + #shows distribution across a range 
  theme_bw(12) +
  labs(x = "Petal length (cm)", y = "Sepal length (cm)")+
  # x axis xustomization with labels specified by sequence
  scale_x_continuous(breaks = seq(0,8,1))
...
# Using base R
par(mfrow= c(1,1))
png("../Figures/Example-baseR.pdf", width = 12, height = 12, units = "cm", res = 600) #..change output folder, set dimensions and resolution accordingly
plot(iris, col = iris$Species)
dev.off() #tells you it's saved
