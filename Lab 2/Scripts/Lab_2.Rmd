...
7 %/% 2 # Integer division (how many ven times does the number go into the other)
...
class(extinct)
class(extant)
av_lifespan <- as.character(av_lifespan)
is.character(av_lifespan)
...
body_mass[length(body_mass)] # gives you last value in index
...
```{r}
# Your code here:
trees <- data.frame(species=c("Oak", "Maple", "Pine", "Birch"),
                    height_m=c(25, 20, 30, 18),
                    is_evergreen=c(FALSE, FALSE, TRUE, FALSE))
trees$height_ft <- trees$height_m*3.28 #converting m to ft
trees[trees$height_m > 20, ] #trees taller than 20m
mean(trees$height_m) #average height of all trees
...
```{r}
# Your code here:
sum(is.na(measurements$length)) #1
median(!is.na(measurements$length)) #2
measurements <- na.omit(measurements) #3
measurements
